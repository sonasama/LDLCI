...
[num_data,num_feature_origin]= size(features);
features(:, num_feature_origin + 1) = 1;
% transform 1/-1 => 1/0
% target=normalize(labels);
% ind = find(target == -1);
% target(ind) = 0;
...
for t = 1:para.rep
    ...
    [X_train, Y_train, X_test, Y_test] = generateCVSet( features,labels,randorder,t,para.rep );
...
%     toc;
    ...
    pre_labels(find(test_outputs<0.5))=0;
    pre_labels(find(test_outputs>0.5))=1;
    
    ind_y = find(Y_test == 0);
    Y_test(ind_y) = -1;
%     TT = EvaluationAll(pre_labels',test_outputs',Y_test');
%     Result_CLML(:,t) = EvaluationAll(pre_labels,test_outputs,Y_test);
    [name,distance] = computeMeasures(test_outputs,  Y_test);
    dis(t,:)=distance;
    %PRO(:,t) = TT;
