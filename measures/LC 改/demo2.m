...
global S1
    ...
    for iter = 1 : para.NIter%开始迭代
        for k = 1 : label_dim%对每个标签
            ...
            D = diag(d);
            W(:,k) = (X_train'*X_train+para.lamda3*D+0.01*P(:,k)'*P(:,k))\(X_train'*P(:,k));%公式11？
            Wwp=zeros(train_num);
            Wwp=double(Wwp);
            ...
            P(:,k) = (Lwp+para.lamda1*Lwn+para.lamda2*Wb+para.lamda4*T*T'+0.01*W(:,k)'*W(:,k))\(X_train*W(:,k)+X_train*W(:,k));%公式14"？更新D(U*U+Lwp+para.lamda1*Lwn+para.lamda2*Wb+W(:,k)'*W(:,k)+T*T')\(U*U*X_train*W(:,k)+X_train*W(:,k));
